...
work_dir = './'
task_name = 'MRI_brain-tumor_32_images'
# prepare SAM model
model_type = 'vit_b'
...
npz_tr_path = 'medsam_data/32_train_set/MRI_brain-tumor/train'
work_dir = './'
task_name = 'MRI_brain-tumor_32_images_DiceFocalLoss'
# prepare SAM model
model_type = 'vit_b'
checkpoint = 'SAM/sam_vit_b_01ec64.pth'
device = 'cuda:0'
...
npz_ts_path = 'medsam_data/32_train_set/MRI_brain-tumor/test'
...
sam_dice = []
medsam_dice = []
for case in range(len(test_npzs)):
    npz = np.load(join(npz_ts_path, test_npzs[case]))
    imgs = npz['imgs']
    gts = npz['gts']
