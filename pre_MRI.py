...
parser.add_argument('--anatomy', type=str, default='brain-tumor', help='anatomy')
parser.add_argument('--img_name_suffix', type=str, default='_t2.nii', help='image name suffix')
parser.add_argument('--label_id', type=int, default=1, help='label id')
parser.add_argument('--prefix', type=str, default='MRI_brain-tumor', help='prefix')
...
names = [name for name in names if name.split(".")[-1] == 'nii']
names = [name for name in names if not os.path.exists(join(args.npz_path, prefix + '_' + name.split('.nii')[0]+'.npz'))]
...
def preprocess_ct(gt_path, nii_path, gt_name, image_name, label_id, image_size, sam_model, device):
    ...
    #Uriel - chose which labels
    #gt_data = np.uint8(gt_data==label_id)
    gt_data = np.logical_or.reduce((gt_data==1, gt_data==2, gt_data==4)).astype(int)
...
for name in tqdm(train_names):
    ...
    if len(imgs)>1:
        ...
        np.savez_compressed(join(save_path_tr, prefix + '_' + gt_name.split('.nii')[0]+'.npz'), imgs=imgs, gts=gts, img_embeddings=img_embeddings)
...
for name in tqdm(test_names):
    ...
    if len(imgs)>1:
        ...
        gts = np.stack(gts, axis=0) # (n, 256, 256)
        #img_embeddings = np.stack(img_embeddings, axis=0) # (n, 1, 256, 64, 64)
        np.savez_compressed(join(save_path_ts, prefix + '_' + gt_name.split('.nii')[0]+'.npz'), imgs=imgs, gts=gts)
        # save an example image for sanity check
        idx = np.random.randint(0, imgs.shape[0])
        img_idx = imgs[idx,:,:,:]
        gt_idx = gts[idx,:,:]
